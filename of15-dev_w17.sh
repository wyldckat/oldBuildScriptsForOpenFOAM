...
echo "Starting svn clone and download 3rd party..."
cd $HOME
...
tar -xzf ThirdParty-1.7.1.gtgz --transform 's=ThirdParty-1.7.1=ThirdParty-1.5-dev='


echo "Fix errors..."
#lets plug-in the latest bashrc and settings.sh
cd OpenFOAM-1.5-dev/etc
...
sed -i -e 's_1\.7\.x_1.5-dev_' -e 's_Gcc};_Gcc43};_' bashrc
...
echo 'export FOAM_DEV=1' >> bashrc #dog tag for -dev
...
if [ -n "$arch" -a $arch -eq 32 ]; then
  sed -i -e 's/WM_ARCH_OPTION:=64/WM_ARCH_OPTION:=32/' bashrc
fi
...
#lets fly to the Thirdparty-1.5-dev dir and fix a few things
cd $WM_THIRD_PARTY_DIR

#Fix a few things before things get even busier
find . -name 'options' | grep 'Make/options' | xargs sed -i -e '/^\/\//d'
...
unset WM_PROJECT_VERSION FOAM_INST_DIR WM_COMPILER
. etc/bashrc
echo "Things are going to get hot, 'cause OpenFOAM 1.5-dev is going to build at full speed."
echo "You can track the current progress in another terminal, by running:"
